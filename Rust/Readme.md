...
코딩을 하면서 제일 중요한 기능이라고 한다면, '로그 출력' 이라고 생각한다.
<br />
현재 내가 짜고있는 코드가 적절하게 돌아가는지 확인해야 되기 때문이다.
<br />
<br />

JS에서는 `console.log` 등으로 로그를 출력해서 확인할 수 있지만, Rust는 방법이 다를테니 이번 시간에는 Rust의 로그 출력에 대해 알아보기로 하자.
<br />
<br />

일단 Rust에서는 '내장함수'라고 칭하는 것이 아닌, '**매크로**'라고 칭한다.

> 간단하게 알아보는 **Rust - `println!` 매크로** vs **JS - `console.log` 내장함수**
>
> - Rust에서 println!은 '매크로'라고 한다. 매크로는 컴파일 타임에 확장되어 코드가 처리되는 특수한 구문이다.(자세한 것은 밑에서)
> - JavaScript에서 console.log는 '내장 함수'라고 한다. 이 함수는 런타임에 실행되며, 다양한 타입의 인자를 받아 콘솔에 출력할 수 있다.
